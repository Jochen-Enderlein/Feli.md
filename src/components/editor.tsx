...
export function Editor({ slug, initialContent, allNotes, graphData, backlinks: initialBacklinks, allTags, allMentions }: EditorProps) {
  ...
  const handleSummarize = async (forceRegenerate: boolean = false) => {
    ...
    try {
      const res = await summarizeNoteAction(content);
      if (res.success && res.summary) {
        setSummaryResult(res.summary);
        localStorage.setItem(cacheKey, res.summary);
        setIsSummaryModalOpen(true);
  ...
  return (
    <div className="flex h-full w-full gap-2 md:gap-4 overflow-hidden">
      ...
      >
        <DialogContent className="sm:max-w-[600px] bg-popover border-border overflow-hidden flex flex-col gap-0 p-0">
          <DialogHeader className="p-4 pb-2 border-b border-border/50">
            ...
            <DialogDescription>
              A concise summary generated by your AI provider.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="p-4 pt-2 border-t border-border/50 flex justify-between items-center bg-muted/20">
            <Button
              variant="ghost"
              size="sm"
            ...
            <div className="flex gap-2">
              ...
              <Button 
                onClick={() => {
                  const newContent = `> [!info] AI Summary\n> ${(summaryResult || '').replace(/\n/g, '\n> ')}\n\n${content}`;
                  setContent(newContent);
